...
(function ($, undefined) {
    ...
    function Timepicker() {
        ...
        this._defaults = { // Global defaults for all the time picker instances
            showOn: 'focus',    // 'focus' for popup on focus,
                                // 'click' for open on mouse click, not tab key
    ...
    $.extend(Timepicker.prototype, {
        ...
        _attachments: function (input, inst) {
            ...
            input.unbind('mousedown', this._showTimepicker);
            if (inst.trigger) { inst.trigger.remove(); }
            var showOn = this._get(inst, 'showOn');
            if (showOn == 'focus' || showOn == 'both') { // pop-up time picker when in the marked field
                input.focus(this._showTimepicker);
            } else if (showOn == 'click') {
                input.mousedown(this._showTimepicker);
            }
        ...
        _generateHTML: function (inst) {
            ...
            html += '<tr>';
            if (amPmText != false) {
                html += '<th rowspan="2" class="periods">' + amPmText[0] + '</th>';
            }
            for (h = 0; h <= 5; h++) {
            ...
            html += '</tr><tr>';
            if (amPmText != false) {
                html += '<th rowspan="2" class="periods">' + amPmText[1] + '</th>';
            }
            for (h = 12; h <= 17; h++) {
                html += this._generateHTMLHourCell(inst, h, showPeriod, showLeadingZero);
            }
